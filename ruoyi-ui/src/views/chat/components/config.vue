              ...
              <el-tab-pane :label="$t('AllInOne.me')" name="me" class="tab_content">
                ...
                <div class="tab_list_div" v-if="!findPromptLoading">
                  <div
                    ...
                    v-infinite-scroll="sessionLoading">
...
import { publicPromptList, personalPromptList, findModelParam, saveModelParam, addPrompt, updatePrompt, deletedPrompt, communitySave, releasePrompt } from '@/api/chat';
...
export default {
  ...
  methods: {
    handleQueryPrompt() {
      ...
      personalPromptList({ name: this.promptMeSearchValue }, this.promptPage).then(res => {
        if (res.code === 200) {
        ...
        if (this.promptListCurr) {
          if (this.promptValue.releaseStatus === '0') {
            const subData = {
              ...
              name: this.promptValue.name,
              prompt: this.promptValue.prompt,
              isUpdateRelease: this.promptValue.releaseStatus
            }
            this.promptSaveState = true;
