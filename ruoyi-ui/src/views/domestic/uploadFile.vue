<template>
  <div class="back" align="left">
    ...
    <div class="background">
      <div style="display: flex;flex-direction: column;align-items: center;">
        <div class="left-controller" id="searchDiv" style="min-height:140px;height: 100%;">
                ...
                <el-col :span="8" :lg="8" :md="12" :sm="12" :xs="24">
                  <el-form-item label="所属部门" prop="deptId">
                    <el-input
                      ...
                      @clear="handleQuery"
                      clearable/>
                    <!--                  <treeselect style="width:75%" v-model="queryParams.deptId" :options="deptOptions" :show-count="true"
                                                  placeholder="请选择归属部门"/>-->
                  </el-form-item>
                </el-col>
                <el-col :span="8" :lg="8" :md="12" :sm="12" :xs="24">
          ...
          <div class="card-button" style="background-color: #F9B889;" @click="handleStartTraining(true)">
            <span class="card-button-text">开始训练</span>
            <svg style="position: absolute;bottom:-2px;right: 0;" width="86" height="86" viewBox="0 0 72 72" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
                d="M49.5 9V15.75H22.5V9H4.5V27H22.5V20.25H38.3018C36.8148 22.1866 36.006 24.5584 36 27V45C35.9976 46.7895 35.2857 48.505 34.0203 49.7703C32.755 51.0357 31.0395 51.7476 29.25 51.75H22.5V45H4.5V63H22.5V56.25H29.25C32.2326 56.2464 35.092 55.06 37.201 52.951C39.31 50.842 40.4964 47.9826 40.5 45V27C40.4994 26.114 40.6734 25.2365 41.012 24.4177C41.3506 23.599 41.8471 22.8549 42.4733 22.2281C43.0995 21.6013 43.8431 21.104 44.6615 20.7645C45.4799 20.4251 46.3572 20.2503 47.2432 20.25H49.5V27H67.5V9H49.5ZM18 22.5H9V13.5H18V22.5ZM18 58.5H9V49.5H18V58.5ZM63 22.5H54V13.5H63V22.5Z"
                fill="white"/>
            </svg>
          </div>
        </div>
            ...
            >
              <template slot-scope="scope" align="enter">
                <div
                  ...
                  <el-tag
                    v-if="scope.row.trainDomesticStatus"
                    style="font-size:14px;"
                    :type="scope.row.trainDomesticStatus === 'success' ? 'success'
                  : scope.row.trainDomesticStatus === 'untrained' ? 'primary'
                  : scope.row.trainDomesticStatus === 'training' ? 'warning'
                  : scope.row.trainDomesticStatus === 'error' ? 'danger' : 'info'">
                    {{ statusOptions.find(item => item.value === scope.row.trainDomesticStatus).label }}
                    <i class="el-icon-loading" style="font-size:16px;" v-if="scope.row.trainDomesticStatus === 'training'"/>
                  </el-tag>
                  <span v-else> - </span>
                  <div style="position: absolute;right:0;top:0;">
                    <el-tooltip v-if="scope.row.trainDomesticStatus === 'error'" effect="light"
    ...
    >
      ...
      <div style="position: absolute;right: 20px;top:16px;z-index:200;font-size: 20px;cursor: pointer" v-show="!isUploading"
...
import {delFile, fileUpload, listFile, updateFile} from "@/api/data/uploadFile";
import {getToken} from '@/utils/auth';
import {deptTreeSelect} from "@/api/system/user";
import Treeselect from "@riophae/vue-treeselect";
import "@riophae/vue-treeselect/dist/vue-treeselect.css";
import {checkIsTrained} from "@/api/data/uploadFile";
import {checkIsDomesticTrained} from "../../api/data/uploadFile";
...
export default {
  ...
  data() {
    return {
      ...
      //   type: "pdf",
      //   deptid: '研发部门',
      //   uploadtime: '2023-05-04',
      //   imageurl: '',
      //   name: "Personnel Handbook.pdf"
      // },
      //   {
      //     textid: 2,
      //     type: "pdf",
      ...
      deptOptions: [],
      uploadList: [],
      configList: ['单个文件大小不超过50MB', '文件名不得包含特殊字符', '当前仅支持 pdf xlsx xls 文件，doc/docx 文件上传后会自动转为pdf'],
      isUploading: false,
  ...
  methods: {
    ...
    selectable(row, index) {
      return row.trainDomesticStatus !== 'training';
    },
    ...
    handleStartTraining(isRobert) {

      checkIsDomesticTrained().then(response => {
        if (response.data) {
          ...
          let list = this.multipleSelection.map(item => {
            return {id: item.id, name: item.blobFileName, suffix: item.suffix, blobUrl: item.blobUrl}
          });
          ...
          let data = {
            ...
            trainType: isRobert ? 'Robert' : 'Embedding',
          }
          ...
          axios.post(this.baseUrl + '/datasetupload', data, {
            headers: {
    ...
    handleStartTrainingDomestic() {
      ...
      axios.post(this.baseUrl + '/datasetupload', data, {  //qcl 请修改字符串内的接口名称
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer ' + getToken()
        }
    ...
    uploadFile(param) {
      if (param.file.size > 1024 * 1024 * 50) {
