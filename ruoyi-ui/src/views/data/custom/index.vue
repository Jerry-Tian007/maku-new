      ...
      >
        ...
        <div class="dialog-container">
          ...
          >
            ...
            <!--历史列表-->
            <div
              v-for="(item,index) in latestHistoryList"
              :key="index"
              style="padding:12px;"
                ...
                <div
                  ...
                  <span v-for="(data,index2) in item.answers" :key="index2">
                    ...
                    <span
                      ...
                      :key="file"
                      v-if="(item.citation.findIndex(item => item.name === file) + 1) >0"
                      @click="openFile(item,item.citation.find(item => item.name === file).originalFileUrl,item.citation.find(item => item.name === file).page,item.citation.find(item => item.name === file).fileSuffix,index)"
                      class="index-span">
                ...
                >
                  <div class="file-tag-container">
                    ...
                    <span
                      ...
                      :key="index2">
                    <el-tag
                      @click="openFile(item,file.originalFileUrl,file.page,file.fileSuffix,index)"
                      size="small"
            ...
            <div v-if="latestHistoryList.length>0" align="center">
              <div style="color: #999093;font-size: 14px;padding-bottom: 10px;">- 以上内容为近期的问答记录,不参与新的问答 -</div>
            </div>
            ...
            >
              ...
              >
                ...
                >
                  <div class="file-tag-container">
                    <span style="font-weight:600;font-size:14px;">可能的参考文件：</span>
                    <span
                      v-for="(file,index2) in item.citationExcept"
                      :key="index2">
                    <el-tag
                      @click="openFile(item,file.originalFileUrl,file.page,file.fileSuffix,index)"
                      size="small"
              ...
              <div v-if="item.answerAnalysis==='NO'">
                <el-card
                  v-for="(business, index2) in businessOptions"
                  :key="index2"
                  class="follow-card"
                >
                  <div style="display: flex;align-items: center;" @click="handleKBRetry(business, index)"
                       :title="business.label">
                    <div style="white-space: nowrap;
                    overflow:hidden;
                    text-overflow:ellipsis;">{{ business.label }}
                    </div>
...
import {addKBHistory, updateKBHistory} from "@/api/data/history";
import {fileDownloadPPT, fileDownloadWord} from "@/api/data/uploadFile";
import {identityChatgptsummaryLatest} from "../../../api/chatGPT/chatgptsummary";
import {listLatestKBHistoey} from "../../../api/data/history";
...
export default {
    ...
    return {
      ...
      historyListOld: [
        ...
        },
        {
          ...
          "answerAnalysis": 'NO',
          "citation": [{
            "name": "262-242-1689910824060_8.pptx",
            "originalFileName": "Public building operation enterprises_8.pptx",
            "originalFileUrl": /*"https://nygpt.blob.core.windows.net/zqlblobsource/gptFile/262/242/262-242-1689910824060.pdf?sv=2020-10-02&se=2033-07-21T03%3A40%3A24Z&sr=b&sp=r&sig=a%2BEzp3QNdq1zOiIhDPygxPo6vq%2Ff%2B8FQbBG8ltp0Uyk%3D" +*/
              'https://oatest1-awss3.s3.cn-northwest-1.amazonaws.com.cn/usr/weaver/ecology/filesystem/202310/J/a6697fa8-9f37-40a1-84e4-d3a6a76549ef?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Date=20231008T075946Z&X-Amz-SignedHeaders=host&X-Amz-Expires=86400&X-Amz-Credential=AKIATZY25J4XZLOZAJWL%2F20231008%2Fcn-northwest-1%2Fs3%2Faws4_request&X-Amz-Signature=44f316cdb730e5c21842f93a158dee6a770e0c0f243962841b06a2dc318ad2ac',
            "url": "/gptData/contentfile?path=262-242-1689910824060_8.pptx",
            "page": "8",
  ...
  mounted() {
    this.sessionMark = new Date().getTime() +
      Math.round(Math.random() * 10) +
      Math.round(Math.random() * 10) +
      Math.round(Math.random() * 10);

    let obj = JSON.parse(localStorage.getItem("query"));

    this.userIdentity.businessId = obj.businessId.map(item => {
      return Number(item)
    });
    this.userIdentity.identityId = obj.identityId;
    this.userIdentity.identityName = obj.identityName;
    this.sendData = '';
    this.token = getToken();
  ...
  methods: {
    getLatestHistoryList() {
      this.latestHistoryList = [];
      listLatestKBHistoey({createBy: this.userIdentity.identityId}).then(res => {
        if (res.code === 200) {
          let list = res.rows;
          list.forEach(item => {
            this.latestHistoryList.push({
              role: 'user',
              text: item.question,
            ...
            obj.text = response.answer;
            let originalFileNames = response.originalFileNames;
            let blobUrl = response.blobUrl;
            let fileSuffix = response.fileSuffix;

            let tmp = response.data_points;
            let citation = [];
            let citationExcept = [];
            tmp.forEach((item, index) => {
              let list = item.split(': ');
              if (matches != null) {
                if (matches.indexOf('[' + list[0] + ']') != -1) {
                  let data = {
                    name: list[0],
                    originalFileName: originalFileNames[index],
                    originalFileUrl: blobUrl[index],
                    fileSuffix: fileSuffix[index],
            ...
            if (n === citation.length) {
              ...
              obj.hasNoFiles = false;
            }
            obj.citation = citation;
            obj.citationExcept = citationExcept;
            obj.follow = response.hiddenQuesion || [];
            obj.tokenUsage = response.tokens;
            obj.answerAnalysis = response.answer_analysis;

            this.latestHistoryList.push(obj);
          })
        }
      });
    },
    ...
    //打开侧边栏-查看文件
    openFile(data, url, page, fileSuffix, index) {
      ...
      this.pageNum = page;
      let formatUrl = url.replace('https://oatest1-awss3.s3.cn-northwest-1.amazonaws.com.cn', process.env.VUE_APP_FILE_URL);
      ...
      if (fileSuffix === 'pdf') {
        ...
        fileDownloadWord(encodeURIComponent(url)).then(res => {
          let blobUrl = window.URL.createObjectURL(res);
          this.iframeUrl = '/build/pdf/web/viewer.html?file=' + encodeURIComponent(blobUrl) + '#page=' + this.pageNum;
        })
      } else if (['ppt', 'pptx'].indexOf(fileSuffix) !== -1) {
        fileDownloadPPT(encodeURIComponent(url)).then(res => {
          let blobUrl = window.URL.createObjectURL(res);
          this.iframeUrl = '/build/pdf/web/viewer.html?file=' + encodeURIComponent(blobUrl) + '#page=' + this.pageNum;
        })
      } else {
    ...
    changeIframeShowStatus(data, index) {
      ...
      } else if (data.citationExcept.length > 0) {
        data.citationExcept.forEach(item => {
          for (let i = 0; i < item.content.length; i++) {
            this.contentList.push({
              name: item.name,
              originalFileName: item.originalFileName,
    ...
    sendRequest() {
      ...
      axios.post(this.baseUrl + '/chat', data, {
        ...
        .then(response => {
          ...
          let fileSuffix = response.data.fileSuffix;
          ...
          this.historyList[index].citationExcept = citationExcept;
          this.historyList[index].follow = response.data.hiddenQuesion || [];
          this.historyList[index].tokenUsage = response.data.tokens;
          this.historyList[index].answerAnalysis = response.data.answer_analysis;
          if (index === 1) {
            this.historyList[index].answerAnalysis = 'NO';
          }

          his[his.length - 1].bot = response.data.answer;
          this.messages = JSON.parse(JSON.stringify(his));
          this.handleAddHistory(response.data.tokens);
          this.isSend = false;
        })
        .catch(error => {
          let obj = JSON.parse(localStorage.getItem("query"));
    ...
    handleAddHistory(tokens) {
      addKBHistory({
        ...this.singleQA,
        type: 'Embedding',
        isSolve: 'chatting',
        sessionMark: this.sessionMark,
        tokenUsage: tokens || 0,
      })
        .then(res => {
          if (res.code !== 200) {
    ...
    handleGood(isGood, item) {
      ...
      updateKBHistory(data, isGood).then(res => {
        item.isGood = isGood;
    ...
    handleKBRetry(business, index) {
      this.$confirm('您选择了 【' + business.label + '】，是否重新提问？', '提示', {
        confirmButtonText: '是',
        cancelButtonText: '否',
        type: 'warning'
      }).then(() => {
        this.userIdentity.businessId = [business.value];
        this.sendData = this.historyList[index - 1].text;
        this.isSend = false;
        this.handleSendData();
      }).catch(() => {
