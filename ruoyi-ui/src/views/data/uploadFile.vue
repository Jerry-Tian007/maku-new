              ...
              <el-row style="width:100%;height:fit-content;">
                ...
                <el-col :span="8" :lg="8" :md="12" :sm="12" :xs="24">
                  ...
                  <el-button
                    ...
                    @click="handleExport"
                    size="medium"
                    v-hasPermi="['gpt:userblob:export']"
                    round
                  >导出
                  </el-button>
                </el-col>
        ...
        <template v-if="!training">
          <el-col :span="1.5">
            ...
            <el-button
              ...
              :disabled="multiple"
              @click="handleDelete"
              v-hasPermi="['gpt:userblob:remove']"
              style="margin: 5px"
            >删除</el-button>
          </el-col>
          <el-table
            ...
            <el-dialog
              ...
              <div style="">
                <el-form :model="editForm"  ref="editForm" label-width="100px">
                  ...
                  <el-form-item label="用户权限" prop="rolePermission">
                    <el-select
        ...
        <div style="flex:1;background-color: white">
          ...
          <div style="padding: 0 40px;display: flex; align-items: center;justify-content: space-between;">
            <div style="display: flex; align-items: center;">
              <el-select
                v-model="queryFileParams.areaPermission"
                placeholder="请选择上传国家"
                multiple
                filterable
                size="small"
...
<!--                @change="handSelectChange"-->
              ...
              <el-upload
                ...
                :limit="handleCheckUnlimited() ? 100 : 50"
              ...
              <el-select
                v-model="queryFileParams.rolePermission"
                placeholder="请选择用户"
                multiple
                size="small"
                style="margin: 10px"
...
export default {
  ...
  data() {
    return {
      ...
      deptOptions: [],
      uploadList: [],
      configListUnlimited: ['单个文件大小不超过50MB', '文件名不得包含特殊字符', '当前仅支持 pdf xlsx xls 文件，doc/docx 文件上传后会自动转为pdf,不支持ppt文件'],
      configList: ['单个文件大小不超过 50MB，文件个数一般不建议不超过10个', '文件名不得包含特殊字符', '当前仅支持 pdf xlsx xls 文件,不支持ppt文件,doc/docx 文件上传后会自动转为pdf'],
      isUploading: false,
  ...
  methods: {
      ...
      checkIsTrained().then(response => {
        if (response.data) {
          ...
          let list = this.multipleSelection.map(item => {
            ...
            let modifiedroleList = item.rolePermission.split(',').join(';');
            return {
              id: item.id,
              ...
              roleList: modifiedroleList
            };
          });

          if (list.some(item => !item.regionList || !item.roleList)) {
            this.$message.warning('文件的国家或角色数据有空情况，无法进行训练，请先修改');
            return;
          }
    ...
    showEditDialog() {
      ...
      if (this.selectedRows.length === 1) {
        ...
        const areaPermission = selectedRow.areaPermission.split(';');
        const rolePermission = selectedRow.rolePermission.split(';');
          // 设置默认值
          this.editForm = {
            areaPermission,
            rolePermission
    ...
    saveEditForm() {
      ...
      //状态
      const trainStatusList = this.selectedRows.map(row => row.trainStatus)
      if (this.editForm.areaPermission.length === 0) {
        this.editForm.areaPermission = this.areaOptions.map(item => item.value);
      }
      if (this.editForm.rolePermission.length === 0) {
        this.editForm.rolePermission = this.rolePermissionOptions.map(item => item.value);
      }
      const formData = {
        idList: ids,
        areaPermission: this.editForm.areaPermission.join(';'),
        rolePermission: this.editForm.rolePermission.join(';'),
        trainStatusList:trainStatusList
      };
    ...
    beforeUpload(file) {
      ...
      if (fileType === 'application/vnd.ms-powerpoint' || fileType === 'application/vnd.openxmlformats-officedocument.presentationml.presentation') {
        this.$message.error('不允许上传 PPT 或 PPTX 文件');
        this.$refs.uploadFileButton.handleRemove(file); // 移除文件
        this.isUploading = false; // 设置上传状态为 false
        return false; // 阻止文件上传
      }
      return true; // 允许文件上传
    },
    ...
    uploadFile(param) {
      ...
      // 如果 areaPermission,rolePermission 为空，则将所有选项赋值给它
      if (this.editForm.areaPermission.length === 0) {
        this.editForm.areaPermission = this.areaOptions.map(item => item.value);
      }
      if (this.editForm.rolePermission.length === 0) {
      ...
      formData.append('areaPermission',  this.editForm.areaPermission.join(';'));
      formData.append('rolePermission',this.editForm.rolePermission.join(';'));
      ...
      fileUpload(formData).then(res => {
        ...
        this.uploadComplete(); // 设置上传状态为 false
      });
    },
    uploadComplete() {
    ...
    handleChange(file, fileList) {
      ...
      this.uploadList = [];
      let size = this.handleCheckUnlimited() ? 1024 : 50;
    ...
    handleExport() {
      ...
      const data = {
      areaPermission: this.queryParams.areaPermission.join(";"),
      rolePermission:this.queryParams.rolePermission.join(";"),
      }
      this.download('gpt/userblob/export', {
        ...this.queryParams,
        ...data,
      }, `contractinfo_${dateTime}.xlsx`);
    },
