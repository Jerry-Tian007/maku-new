  ...
  <div class="back">
    ...
    <div class="backgrond">
      <div
        ...
        :style="(isMobile ? 'width:100%;' : '') +'height:50px;background:#00000000;display: flex;flex-direction: column;align-items: flex-end;'"
      >
        <div style="display: flex;height:50px;align-items: center;margin-right: 2.5%;padding-top:20px;">
          <div
            v-if="isMobile"
          ...
          >
            ...
            />
            <span style="font-size: 16px;">会话列表</span>
          </div>
          <el-tooltip v-if="isMobile" effect="light" content="请及时清空对话，对话历史过多会影响使用效果" :placement="isMobile? 'bottom' : 'left'">
            <div
            ...
            >
              <i
                ...
                style="font-size: 18px;margin-right:4px;"
              />
              <span style="font-size: 18px;">清空对话</span>
            </div>
          </el-tooltip>
          <div
            class="clear-button-clear"
          ...
          >
            ...
            />
            <span style="font-size: 18px;">清空对话</span>
            <div v-if="!isMobile" style="font-size:12px;margin-top:4px;color:#a0a0a0;">请及时清空对话，对话历史过多会影响使用效果</div>
          </div>
          <div
            v-if="isMobile"
            class="clear-button-setting"
      ...
      <div
        ...
        style=""
      >
        <div :style="isMobile ? 'flex:0.05;' : 'width:160px;display:flex;flex-direction: column;'" align="center" :class="isMobile?'':'session-list'">
          <template v-if="!isMobile">
            ...
            <div style="overflow-y:auto;" v-infinite-scroll="load">
              ...
              >{{ getName(item) || '新会话'}}</span>
                <div class="session-delete" @click="handleDeleteSession(item,index)"><i class="el-icon-delete"></i></div>
              </div>
        ...
        <div class="dialog-container"
             :style="isMobile ? 'padding-top:60px;' : 'padding-top:10px;'">
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <svg
                        v-if="item.id == undefined && item.isGood"
                        width="18"
                        height="18"
                        viewBox="0 0 48 48"
                        fill="none"
        ...
        <div
          :style="isMobile ? 'flex:0.05;' : 'flex:0.4;width:100%;height:100%;display: flex;align-items: center;'"
        >
          <div
            v-if="!isMobile"
            style="display: flex;flex-direction: column;align-items: center;width: 100%;height:75%;"
          >
    ...
    >
      <div
        class="session-card"
        style="border-color:#606266;margin-bottom: 20px;"
      >
        <div @click="handleAddSession();sessionVal=false" style="width: 100%;" align="center"><i class="el-icon-plus"></i></div>
      </div>
      <div
        class="session-card"
        v-for="(item,index) in sessionList"
        :key="index"
        :style="item.id === sessionId ?  'border-color:#1c84c6;border-width:2px;' : 'border-color:#606266'"
      >
              <span
        ...
        >
          ...
          >
            <el-form-item>
              ...
              >
                <el-option
                  label="gpt-35-turbo"
                  value="gpt-35-turbo"
                  v-has-permi="['gpt:chatgptrecord:35']"
                />
                <el-option
                  label="gpt-4"
                  value="gpt-4"
                  v-has-permi="['gpt:chatgptrecord:4']"
                />
              </el-select>
            </el-form-item>
            <el-form-item>
            ...
            <el-form-item>
              <template slot="label">
                <el-tooltip
                  :content="description.frequency_penalty"
                  placement="top"
                >
                          <span>频率惩罚(-2~2)<i
                            class="el-icon-info"
            ...
            <el-form-item>
              <template slot="label">
                <el-tooltip
                  :content="description.presence_penalty"
                  placement="top"
                >
                          <span>存在惩罚(-2~2)<i
                            class="el-icon-info"
...
import { chatQuestion } from "@/api/chatGPT/chat";
...
export default {
  ...
  methods: {
    ...
    handleSelectRole(id) {
      ...
      this.handleChangeSetting();
      const i = id - 1;
      if(i < 0) {this.systemMsg = '你是一个喜欢使用中文的助手';return;};
      this.systemMsg = this.roleList[i].rule;
    },
    formatterMessage(){
      let x = this.historyList.map(item => {return {role: item.role === 'gpt' ? 'assistant' : 'user',content: item.text}});
    ...
    load(){
      ...
      if (this.sessionList.length-1 === this.total && this.sessionList.findIndex(item => item.id === 0) !== -1){
        this.isLoadingSession = false;
        return;
      }
    ...
    getSystem() {
      return this.systemMsg + (this.content != '' ? ' 问题背景： ' + this.context : '')
        + (this.isTextNum ? ' 请在' + this.textNum + '字以内简略叙述完毕 ' : '') + (this.isSteps ? '请按步骤，分簿、分点告诉我' : '');
    },
    clearDialog() {
      this.isError = false;
      this.isSend = false;
    ...
    sendRequest() {
      ...
      chatQuestion(url, data) //post请求，传递参数text
        ...
        .catch(error => {
          ...
          this.historyList[index].role = 'gpt';
          this.historyList[index].isError = true;
          this.historyList[index].errorText = 'Error: Somthing went wrong.';
    ...
    handleImage(data) {
      let index = this.historyList.length - 1;
      axios.get(this.baseUrl + '/nydalle?prompt=' + data)
        .then(response => {
          this.historyList[index].id = new Date().getTime();
          this.historyList[index].text = '![avatar](' + response.image_url + ')';
          this.historyList[index].timestamp = new Date().getTime();
          this.signleQA.answer = '![avatar](' + response.image_url + ')';
          this.handleAddHistory();
        }).catch(err => {
        console.log(err);
        this.historyList[index].role = 'gpt';
    ...
    handleGood(isGood, item) {
      ...
      updateHistory(data, isGood).then(res => {
        item.id = null;
        item.isGood = isGood;
        let obj = this.onlyAddList.find(obj => obj.id === item.id);
        obj.isGood = isGood;
      }).catch(err => {
        console.log(err);
      });
    ...
    handleDeleteSession(item,index){
      if (item.id === 0) {
        if (this.sessionList.length < 2) {
          ...
          this.sessionList.splice(index,1);
          this.sessionId = this.sessionList[this.sessionList.length-1].id;
          let list = this.sessionList[this.sessionList.length-1].history;
          this.onlyAddList = JSON.parse(list);
          this.historyList = JSON.parse(list);
        }
        return;
    ...
    handleUpdateSession(id,listString){
      if (id===0)return;
      updateSession({id: id, history: listString}).then(res => {
        if (res.code === 200){
        } else {
          this.$message.error(res.msg);
        }
